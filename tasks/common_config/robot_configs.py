...
from robots.unitree import G129_CFG_WITH_DEX1_BASE_FIX,G129_CFG_WITH_DEX3_BASE_FIX, G1_CFG, G129_CFG_WITH_INSPIRE_HAND
...
class G1RobotJointTemplates:
    ...
    @classmethod
    def get_hand_joints(cls, hand_type: Literal["gripper", "dex3", "ftp", "inspire"] = "gripper") -> Dict[str, float]:
        ...
        if hand_type == "gripper":
            ...
            raise ValueError(f"Unsupported hand type: {hand_type}. Supported: 'gripper', 'dex3', 'ftp', 'inspire'")
...
class G129dofRobotBaseCfg:
    ...
    def get_base_config(
        ...
        hand_type: Literal["gripper", "dex3", "ftp", "inspire"] = "gripper",
        base_config = None,
...
class G1RobotPresets:
    ...
    @classmethod
    def g1_29dof_ftp_base_fix(cls,init_pos: Tuple[float, float, float] = (-0.15, 0.0, 0.76),
        init_rot: Tuple[float, float, float, float] = (0.7071, 0, 0, 0.7071)) -> ArticulationCfg:
        """pick-place task configuration - dftp hand"""
        return G129dofRobotBaseCfg.get_base_config(
            init_pos=init_pos,
            init_rot=init_rot,
            include_waist=False,
